...
public class ImageListView extends ListActivity
{
	  ...
	  protected void onCreate(Bundle icicle)
	  {
        ...
        {
        	 Log.i("TAG", "obviously, you clicked item is usbhost");
   		    //scan all image
	  ...
	  @Override
		protected void onResume() {
			...
			if(!path.equals(Common.SD_CARD_PATH))
			{
				return;  //nothing
			}	
	        ...
	        mReceiver = new BroadcastReceiver() {
	            ...
	            public void onReceive(Context context, Intent intent) {
	            	...
	            	try
		            {
		               getThumbnailsPhotosInfo(Common.SD_CARD_PATH);
      ...
      private void getThumbnailsPhotosInfo(String path)
      {
	      ...
	      for(Iterator<?> it = albums.entrySet().iterator(); it.hasNext(); )
	      {
	    	   ...
	    	   if(album!=null&&album.size()>0)
	    	   {
    		       ...
    		       info.tag=list;
    		       if(albumpath.contains(path) && !path.equals(Common.INTERNAL_MEMORY_PATH)){///sdcard/external_sdcard/
    		    		  bitmaps.add(info);
    		       }
    		       if(!albumpath.contains(Common.SD_CARD_PATH) && path.equals(Common.INTERNAL_MEMORY_PATH)){///sdcard/
 		    		  bitmaps.add(info);
    		       }
